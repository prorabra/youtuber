...
function bindSlideToggle(trigger, boxBody, content, openClass) {
    let button = {
        'elment': document.querySelector(trigger),
        'active': false
    };
    const box = document.querySelector(boxBody),
            boxContent = document.querySelector(content);

    button.elment.addEventListener('click', () => {
        if (button.active === false) {  //проверяем меню на не активность
            button.active = true;       // если не активна то делаем активной
...
function switchMode(){
    if (night === false) {
        night = true;
        // document.body.style.backgroundColor = '#000'; // один вариант
